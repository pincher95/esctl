...
var deleteAliasCmd = &cobra.Command{
	...
	esctl delete alias --name my-alias --indices=my-index
	`),
	RunE: func(cmd *cobra.Command, args []string) error {
		return alias.HandleAliasRemove(cmd.Context(), deleteAliasName, deleteAliasIndices)
	},
}

func init() {
	deleteAliasCmd.Flags().StringVar(&deleteAliasName, "name", "", "Alias name")
	deleteAliasCmd.Flags().StringVar(&deleteAliasIndices, "indices", "", "Comma-separated list of indices to remove the alias from")
	deleteAliasCmd.MarkFlagRequired("name")
	deleteAliasCmd.MarkFlagRequired("indices")
}

var deleteAliasName string
